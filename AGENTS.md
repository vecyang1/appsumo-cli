...
Release notes live in `docs/releases/<tag>.md` and the GitHub release body is
published **from that file**:

```bash
gh release create v0.2.0 --title "..." --notes-file docs/releases/v0.2.0.md --verify-tag
```

They are tracked rather than written straight into the release form so that they
fall inside gates this repository already has. `TestDocumentedCommandsParse` then
checks every `appsumo ...` string in them against the real parser, and
`TestTrackedMarkdownHasNoCredentialShapedLiterals` scans them for pasted secrets.
Notes typed into the GitHub form are inside no gate at all — which is how a
release announcement comes to document a flag that was renamed two versions ago.
Both gates were confirmed to fail on `docs/releases/v0.2.0.md` specifically, not
merely to count it.

**Tag the commit CI passed on, not the branch.** `main` can move between the run
finishing and the tag being cut, and a tag is the one ref people install from:

```bash
gh run view <run-id> --json status,conclusion,headSha
```

Read `headSha` back and tag *that*. `git tag -a v0.2.0 <sha>`, then confirm
`git rev-list -n1 <tag>` equals it before pushing. "CI is green on main" is a
claim about a branch; a tag needs a claim about a commit.

Then verify from outside: `go install github.com/vecyang1/appsumo-cli/cmd/appsumo@<tag>`
in a temp `GOBIN`, run it from a directory that is not the checkout, and confirm
`go version -m` reports the tag rather than a pseudo-version.

There is no `v0.1.0` tag. That release exists only as a `CHANGELOG.md` section;
the first tagged release is `v0.2.0`.
